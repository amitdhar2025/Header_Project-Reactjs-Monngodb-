...
const RichTextEditorNew = () => {
  ...
  const handleSave = async () => {
    // Allow saving even if name or content is empty
    try {
      const formData = new FormData();
      formData.append("name", name || "");
      formData.append("content", content || "");

      // Assuming image file is selected via an input element with id 'imageInput'
  ...
  const removeTableFormat = () => {
    ...
    while (tables.length > 0) {
      ...
      const textNode = document.createTextNode(table.innerText);
  ...
  return (
    <div className="w-full max-w-full container mx-auto md:px-20 px-4 my-8" style={{ width: "100%", border: "none", outline: "none" }}>
      ...
      />
      {/* Removed file input element as per user request */}
      ...
      {allRichTexts.length > 0 && (
        <div className="mt-8">
          ...
          <table className="min-w-full border-collapse border-0">
            <thead>
              <tr>
                <th className="px-4 py-2 border-0">Name</th>
                <th className="px-4 py-2 border-0">Content</th>
                {/* Removed Image column as it is not stored in DB */}
                <th className="px-4 py-2 border-0">Actions</th>
              </tr>
            </thead>
            <tbody>
              {allRichTexts.map((entry) => (
                <tr key={entry._id}>
                  <td className="px-4 py-2 border-0">{entry.name}</td>
                  <td className="px-4 py-2 border-0" dangerouslySetInnerHTML={{ __html: entry.content }}></td>
                  {/* Removed Image cell as it is not stored in DB */}
